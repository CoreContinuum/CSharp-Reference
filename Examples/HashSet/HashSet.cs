...
namespace HashSet
{
  internal class HashSet
  {
    static void Main(string[] args)
    {
      try
      {
        Console.WriteLine("HashSet Demo, Loading via database or programmaticaly");
        Console.WriteLine("");
        ...
        if (inputLDP == "0")
        {
          try
          {
            #region Load HashSet<T> via Database
        ...
        if (errorBln == false)
        {
          try
          {
            #region Display HashSet<T> values unsorted (original)
            // Display all of the HashSet<T> values
            Console.WriteLine("Start of HashSet<T> values unsorted (original)...");

            foreach (var item in HSet)
            {
              Console.WriteLine(item);
            }
            Console.WriteLine("End of HashSet<T> values unsorted (original)...");
            ...
            if (errorBln == false)
            {
              try
              {
                ...
                if (inputSort != null)
                {
                  if (inputSort == "A")
                  {
                    var ascList = HSet.OrderBy(x => x);
                    HSet = ascList.ToHashSet();
                  }
                  else
                    if (inputSort == "D")
                  {
                    ...
                    //HSet.Reverse();
                    //or
                    var descList = HSet.OrderByDescending(x => x);
                    HSet = descList.ToHashSet();
                  }
                  else
                  {
                    errorBln = true;
